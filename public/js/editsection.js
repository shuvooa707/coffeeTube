...
function registerSelected() {
    ...
    let videos = document.querySelector("input[name='videos']");
    newVideos = JSON.parse(videos.value);
    let selectedVideos = [...document.querySelectorAll(".selected")].map(function(e){
        newVideos.id.push(e.dataset.id);
    });

    videos.value = JSON.stringify(newVideos);

    selectedVideosObject = [...selected];
    console.log(selectedVideosObject);
}
